...
class BulletinAdmin(ModelAdmin):
    ...
    Features:
    - Display writer name and username
    - Show article count and subscriber count
    - Link to writer profile
    - Search by bulletin title and writer username

    Permissions:
    - Admin-only access
    """

    list_display = ['title', 'writer_display', 'articles_count', 'subscribers_count', 'created_display']

    list_filter = ['created', 'updated']

    search_fields = ['title', 'owner__user__username', 'owner__user__first_name', 'owner__user__last_name']

    readonly_fields = ['created', 'updated', 'writer_profile_link', 'articles_count_display', 'subscribers_count_display']
    ...
    fieldsets = (
        ...
        ('Writer Information', {
            'fields': ('owner', 'writer_profile_link')
        }),
        ('Statistics', {
            'fields': ('articles_count_display', 'subscribers_count_display')
        }),
        ('Dates', {
    ...
    def articles_count(self, obj):
        """Display count of articles in list view."""
        return obj.articles.count()
    articles_count.short_description = 'Articles'

    def subscribers_count(self, obj):
        """Display count of subscribers in list view."""
        return obj.subscribers.count()
    subscribers_count.short_description = 'Subscribers'

    def created_display(self, obj):
        """Display creation date in list view."""
        return obj.created.strftime('%Y-%m-%d')
    created_display.short_description = 'Created'
    created_display.admin_order_field = 'created'

    def writer_profile_link(self, obj):
        """Display link to writer's profile in detail view."""
        profile = obj.owner
        return f'{profile.full_name} (ID: {profile.id}) — @{profile.user.username}'
    writer_profile_link.short_description = 'Writer Profile'

    def articles_count_display(self, obj):
        """Display article count in detail view."""
        return obj.articles.count()
    articles_count_display.short_description = 'Total Articles'

    def subscribers_count_display(self, obj):
        """Display subscriber count in detail view."""
        return obj.subscribers.count()
    subscribers_count_display.short_description = 'Total Subscribers'
